...
public class Glide implements ComponentCallbacks2 {
  ...
  private static void initializeGlide(@NonNull Context context, @NonNull GlideBuilder builder) {
    ...
    //这里是为了兼容以前modules写在manifest里
    List<com.bumptech.glide.module.GlideModule> manifestModules = Collections.emptyList();
    if (annotationGeneratedModule == null || annotationGeneratedModule.isManifestParsingEnabled()) {
      manifestModules = new ManifestParser(applicationContext).parse();
    }
    //这里是为了兼容以前modules写在manifest里，以后将不再兼容
    ...
    //拿到了GeneratedRequestManagerFactory，默认build方法里构建了glideRequest
    RequestManagerRetriever.RequestManagerFactory factory =
        annotationGeneratedModule != null
            ? annotationGeneratedModule.getRequestManagerFactory() : null;
    builder.setRequestManagerFactory(factory);
    //如果manifest和注解中同时配置，注解生效
  ...
  Glide(
     ...
     */
    registry
        .append(ByteBuffer.class, new ByteBufferEncoder()) //将buffer存文件
        .append(InputStream.class, new StreamEncoder(arrayPool))
        /* Bitmaps */
        .append(Registry.BUCKET_BITMAP, ByteBuffer.class, Bitmap.class, byteBufferBitmapDecoder) //将buffer转换成Bitmap
        ...
        .append(
            Registry.BUCKET_BITMAP, Bitmap.class, Bitmap.class, new UnitBitmapDecoder()) //只是做了一个wrapper，实际上不做任何处理
        ...
        /* Drawables */
        .append(Uri.class, Drawable.class, resourceDrawableDecoder)  //Uri转换成Drawable，一般加载资源就用这个
        .append(
            Uri.class, Bitmap.class, new ResourceBitmapDecoder(resourceDrawableDecoder, bitmapPool))
        /* Files */
        .register(new ByteBufferRewinder.Factory())
        .append(File.class, ByteBuffer.class, new ByteBufferFileLoader.Factory())  //将File文件转换成ByteBuffer
